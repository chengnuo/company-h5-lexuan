  ...
  <div class="login">
    ...
    <div class="login__form">
      <div class="mobile">
        <img class="mobile--icon"
             src="../../assets/img/login/icon_user_01@2x (3).png"
             alt="">
    ...
    <div class="login__footer"
      ...
      <van-button class="btn-save"
                  type="danger"
                  @click="handlelogin"
                  :disabled="form.mobile.length>0 && form.password.length>0 ?false:true">登 录</van-button>
      <!-- <p class="describe">
        注册即表示你同意
    ...
    <!-- <div class="company">
      ...
      <div>香港幸福浪莎國際控股集團有限公司 版權所有</div>
    </div> -->


    <!-- 弹出层 -->
    <!-- <van-overlay :show="!isWeiXin" @click="isWeiXin = false">
      <div class="wrapper" @click.stop>
        <div class="isWeiXinBlock" >
          请在微信打开页面
        </div>
      </div>
    </van-overlay> -->
...
import { apiAuthCode, apiCheckCode, apiRegister, apiLangshaShopMobileAuthLogin } from '@/api/login'
...
export default {
  ...
  methods: {
    ...
    getAuthCode() {
      ...
      apiAuthCode(paramsData).then(res => {
        ...
        if (res.code.toString() === '10004' || res.code.toString() === '10014') {
          ...
          // 注册
          const paramsDataActRegister = Object.assign({}, {
            type: 1,
            areaCode: this.checkDialog.number === null ? '86' : this.checkDialog.number,
            mobile: this.form.mobile
          })

          apiAuthCode(paramsDataActRegister).then(resActRegister => {
    ...
    handleCode() {
      ...
      if (reg.test(this.form.mobile) && this.form.password.length >= 1) {
    ...
    login() {
      ...
      apiLangshaShopMobileAuthLogin(data).then(res => {
        Toast.clear();
