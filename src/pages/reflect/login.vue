      ...
      <van-dialog className="inviteCodeShow"
        ...
        <div class="inviteCodeShowBtns">
          ...
          <van-button type="default"
                      ...
                      @click="inviteCodeShowSuccess">确认</van-button>
        </div>
      </van-dialog>
    </div>
    <div v-if="!notShow">
...
export default {
  ...
  methods: {
    ...
    init() {
      ...
      //   }else if (getLoginAuthorization && JSON.stringify(getLoginAuthorization) !== '{}') {
      //   } else {
      //     if(window.location.host == 'test-h5-mpwj.mingpinmao.cn'){
      //       const query = this.$route.query
      //       let inviteCode = query.inviteCode || ''
      //       window.location.href = `${commonJs.getUrl()}/LoginMiddleware?loginSource=${commonJs.payConfig.mingpinwanjiaPAYSource}&inviteCode=${inviteCode}`
      //     }else{
      //       this.$router.push({
      //         name: 'LoginMiddleware',
      //         query: {
      //           loginSource: `${commonJs.payConfig.PAYSource}` // 判断正式/测试环境,
      //         }
      //       }) // 跳转去中间页
      //     }
    ...
    // 登录或注册
    async login() {
      // if(this.form.mobile == '13533900004' && window.location.host == 'h5.mingpinmao.cn')

      const { register, loginMp } = this.$api
      Object.assign(this.form, { inviteCode: this.inviteCode || '' })

      const res = await this.$post(this.type === 1 ? register : loginMp, this.form)

      console.log('2')

      try {
        if (res.code.toString() === '10000') {
          // const redirectUrl = JSON.parse(window.sessionStorage.getItem('redirectUrl')) || {}
          const getInviteCodeRedirectUrlFn = commonJs.getInviteCodeRedirectUrlFn()

          if (this.type === 1) {
            if (window.location.host === 'test-h5-mpmall.mingpinmao.cn') { // 兼容安卓
              this.$router.push({ name: 'JumpSucceed', query: {
                type: 1,
          ...
          } else if (getInviteCodeRedirectUrlFn && getInviteCodeRedirectUrlFn.redirectUrl) { // 跳转逻辑，如果商品详情存在 redirectUrl 就跳转到登录
            ...
            const inviteCode = res.data.inviteCode || ''
            // window.location.href = `${window.location.origin}/gradeAct/ptLevel1?inviteCode=${inviteCode}`
        ...
        } else if (res.code.toString() === '10007') {
          // 用户不存在
          this.inviteCodeShow = true // 注册并且登录
        } else if (res.code.toString() === '10047') {
          // 邀请码不存在
    ...
    // 注册并且登录
    async actRegisterAndLogin() {
      const { actRegister } = this.$api
      ...
      const res = await this.$post(actRegister, data)

      try {
        if (res.code.toString() === '10000') {
          const getInviteCodeRedirectUrlFn = commonJs.getInviteCodeRedirectUrlFn()
          const inviteCode = res.data.inviteCode || this.form.inviteCode || ''
          ...
          // 邀请码不存在
          this.inviteCodeShow = true // 邀请码不存在
        } else {
          this.$toast(res.msg)
        }
      } catch (error) {
